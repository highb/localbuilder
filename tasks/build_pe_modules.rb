...
class BuildPEModulesPackage < TaskHelper
  def task(platforms: nil, **kwargs)
    ...
    Dir.mktmpdir do |dir|
      Dir.chdir dir do
        output, status = Open3.capture2e("git clone -b #{version} --single-branch git@github.com:puppetlabs/pe-modules-vanagon.git")
        ...
        Dir.chdir 'pe-modules-vanagon' do
          ...
          platforms.each do |platform|
            ...
            raise TaskHelper::Error.new("Unable to build PE Installer package for platform #{platform}", 'barr.buildpackages/pe-modules-failed', output) if !status.exitstatus.zero?
